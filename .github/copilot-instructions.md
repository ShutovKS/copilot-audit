...
## Project Overview
TestOps Evolution Forge is an autonomous AI agent system for test generation using Cloud.ru Evolution LLMs. The system uses **LangGraph** for agent orchestration, Playwright for browser automation, and FastAPI + React for the interface.
...
- **final_output** → saves to database
...
### Key Services
- **TestExecutorService** (services/executor.py): Spins up Docker containers (`testops-runner:latest`) to execute tests in isolation
...
### Running Locally
```bash
docker-compose up --build -d  # Full stack: backend (8000), frontend (3000), db (5432), chromadb (8001)
```
...
- SSE streaming from `/api/v1/chat` endpoint (chat.py) yields JSON events: `{type: 'log'|'plan'|'code', content: ...}`
...
- `POST /api/v1/execute`: Triggers Docker test execution
