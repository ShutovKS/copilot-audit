...
const TabButton = ({ active, onClick, children }: { active: boolean, onClick: () => void, children: React.ReactNode }) => (
    ...
    >
        {children}
        {active && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary shadow-[0_-2px_10px_rgba(0,182,122,0.5)]" />}
    </button>
);
...
const Toggle = ({ value, onChange }: { value: boolean, onChange: (v: boolean) => void }) => (
    ...
    >
        <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-transform duration-300 ${value ? 'left-6' : 'left-1'}`} />
    </button>
);
                        ...
                        <div className="p-8 space-y-6">
                            <div className="grid gap-4">
                                <div className="flex items-center justify-between p-4 bg-[#18191d] rounded-xl border border-white/5">
                                    ...
                                    {status ? <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-400 rounded-full text-xs font-medium border border-emerald-500/20"><CheckCircle2 size={14}/> Online</div> 
                                ...
                                <div className="flex items-center justify-between p-4 bg-[#18191d] rounded-xl border border-white/5">
                                    ...
                                    {status?.database === 'connected' ? 
                                        <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-400 rounded-full text-xs font-medium border border-emerald-500/20"><CheckCircle2 size={14}/> Connected</div> :
                                ...
                                <div className="flex items-center justify-between p-4 bg-[#18191d] rounded-xl border border-white/5">
                                    ...
                                    {status?.llm === 'ready' ? 
                                        <div className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-400 rounded-full text-xs font-medium border border-emerald-500/20"><CheckCircle2 size={14}/> Ready</div> :
                                        (loading ? <Loader2 size={18} className="animate-spin text-muted" /> : <XCircle size={18} className="text-error" />)
                                    }
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {AVAILABLE_MODELS.map(model => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold flex items-center gap-2 text-white">
                                                    {model.name}
                                                    {model.isFree && (
                                                        <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/20 text-emerald-400 uppercase tracking-wide border border-emerald-500/20">Free</span>
                                                    )}
                                                </div>
                                                <div className="text-[11px] text-muted flex items-center gap-2 mt-1">
                                                    <span className="px-2 py-0.5 rounded bg-white/5 flex items-center gap-1"><LayoutTemplate size={10}/> {model.context}</span>
                                        ...
                                        {selectedModel === model.id && <div className="w-6 h-6 rounded-full bg-[#00b67a] flex items-center justify-center text-white"><Check size={14} strokeWidth={3} /></div>}
                                    </button>
                                ))}
                            </div>
                        </div>
