...
export const Terminal = ({onOpenSettings}: TerminalProps) => {
	...
	return (
		<div className="flex flex-col gap-4 h-full">
			<div className="bg-[#1f2126] rounded-2xl p-6 shadow-xl border border-white/5 relative overflow-hidden">
				...
				{isIdle ? (
					<div className="flex flex-col items-center justify-center py-4 animate-in fade-in zoom-in duration-500">
						<div
							className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#00b67a]/10 to-blue-500/10 flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(0,182,122,0.1)] border border-white/5">
						...
						<div className="flex justify-between relative px-2 mb-2">
							...
							{STEPS.map((step, index) => {
                                ...
                                `}>
										...
										<div
											className={`absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap flex flex-col items-center transition-all duration-300`}>
                                    <span
	                                    className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? 'text-white' : isError ? 'text-error' : isPast ? 'text-zinc-400' : 'text-muted'}`}>
                                        {step.label}
                                    </span>
										</div>
									</div>
			...
			<div
				className="flex-1 bg-[#1f2126] rounded-2xl p-0 shadow-xl border border-white/5 flex flex-col overflow-hidden transition-all duration-300 hover:border-white/10">
				<div className="p-3 px-4 border-b border-white/5 flex justify-between items-center bg-[#18191d]/30">
					...
					<div className="flex items-center gap-1.5">
						<div
							className={`w-1.5 h-1.5 rounded-full ${status === 'processing' ? 'bg-[#00b67a] animate-pulse' : status === 'error' ? 'bg-error' : 'bg-zinc-700'}`}/>
