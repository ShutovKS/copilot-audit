...
class WebInspector:
    ...
    def _get_element_info(self, tag, include_text=True) -> str:
        ...
        attrs = []
        if tag.get('id'): attrs.append(f"id='{tag.get('id')}'")
        if tag.get('data-testid'): attrs.append(f"data-testid='{tag.get('data-testid')}'")
        if tag.get('name'): attrs.append(f"name='{tag.get('name')}'")
        if tag.get('placeholder'): attrs.append(f"placeholder='{tag.get('placeholder')}'")
        if tag.get('role'): attrs.append(f"role='{tag.get('role')}'")
        if tag.get('type'): attrs.append(f"type='{tag.get('type')}'")
