...
class BrowserManager:
    """
    Singleton manager for a persistent Playwright browser instance.
    Ensures that we don't re-launch the expensive browser process.
    """
    ...
    async def close_browser(cls):
        async with cls._lock:
            if cls._browser and cls._browser.is_connected():
                logger.info("Closing persistent browser instance...")
...
class WebInspector:
    ...
    def _parse_html_to_context(self, html: str) -> str:
        ...
        for tag in soup.find_all(["h1", "h2", "h3", "div"]):
            ...
            if tag.name == "div":
                attrs = (
                    str(tag.attrs.get("class", ""))
                    + str(tag.attrs.get("id", ""))
                    + str(tag.attrs.get("data-testid", ""))
                )
                if not any(
                    k in attrs.lower()
                    for k in ["card", "panel", "form", "container", "wrapper"]
                ):
                    continue
    ...
    def _get_element_info(self, tag, include_text=True) -> str:
        ...
        meaningful_classes = [
            c
            for c in classes
            if any(x in c for x in ["btn", "input", "card", "nav", "menu", "item"])
        ]
        ...
        try:
            ...
            try:
                try:
                    ...
                    logger.error(
                        f"Dry Run: Timeout visiting {url}. Cannot check locators."
                    )
            ...
            logger.error(f"Dry Run failed unexpectedly: {e}", exc_info=True)
