...
class CodeValidator:
    ...
    def validate(code: str) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            if pytest_res.returncode != 0:
                logger.warning(f"⚠️ [Linter] Pytest Collection failed.")
                ...
                if isinstance(child, ast.Call) and isinstance(child.func, ast.Attribute):
                    if isinstance(child.func.value, ast.Name) and child.func.value.id in var_types:
                        ...
                        if method_name not in registry[class_type]:
                            errors.append(f"POM Violation in '{func_node.name}': Method '{method_name}' called on '{var_name}' but is NOT defined in class '{class_type}'.")
    ...
    def _check_class(node: ast.ClassDef, errors: List[str]):
        ...
        if "allure.feature" not in class_decorators: errors.append(f"Class '{node.name}' missing @allure.feature")
        if "allure.story" not in class_decorators: errors.append(f"Class '{node.name}' missing @allure.story")
    ...
    def _check_function(node: ast.FunctionDef, errors: List[str], parent_has_owner: bool):
        ...
        if "allure.title" not in func_decorators: errors.append(f"Test '{node.name}' missing @allure.title")
        if "allure.tag" not in func_decorators: errors.append(f"Test '{node.name}' missing @allure.tag")
        if "allure.link" not in func_decorators: errors.append(f"Test '{node.name}' missing @allure.link (Jira)")
        if "allure.label.priority" not in func_decorators and not CodeValidator._has_label(node, "priority"): errors.append(f"Test '{node.name}' missing @allure.label('priority', ...)")
        if not parent_has_owner and not CodeValidator._has_label(node, "owner"): errors.append(f"Test '{node.name}' missing @allure.label('owner', ...)")

    @staticmethod
    def _get_decorator_names(node: [ast.ClassDef, ast.FunctionDef]) -> Set[str]:
        decorators = set()
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Attribute): decorators.add(f"{decorator.value.id}.{decorator.attr}")
            ...
            if isinstance(decorator, ast.Call) and isinstance(decorator.func, ast.Attribute):
                if decorator.func.value.id == "allure" and decorator.func.attr == "label":
                    if decorator.args:
                        ...
                        if isinstance(first_arg, ast.Constant) and first_arg.value == label_name: return True
        return False
